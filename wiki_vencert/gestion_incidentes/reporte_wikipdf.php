...
<body>
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
        ...
        $seleccionartitu = mysql_query("SELECT * FROM gestion_incidente_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_GIT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM gestion_incidente_articulo_d WHERE '$sesiontitu[ID_GIT]' = ID_TITULO_GIT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_GID'];
...
$dompdf->render();
$dompdf->stream("Wiki_Gestion de Incidentes.pdf");
