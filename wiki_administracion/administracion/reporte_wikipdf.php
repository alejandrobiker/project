...
<body>
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
      ...
      $seleccionartitu = mysql_query("SELECT * FROM procesos_administrativo_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_PAAT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM procesos_administrativo_articulo_d WHERE '$sesiontitu[ID_PAAT]' = ID_TITULO_PAAT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_PAAD'];
...
$dompdf->render();
$dompdf->stream("Wiki_Administracion.pdf");
