...
<body>
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
      ...
      $seleccionartitu = mysql_query("SELECT * FROM pronunciamiento_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_PAT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM pronunciamiento_articulo_d WHERE '$sesiontitu[ID_PAT]' = ID_TITULO_PAT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_PAD'];
...
$dompdf->render();
$dompdf->stream("Wiki_pronunciamientos.pdf");
