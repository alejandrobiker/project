...
<body>
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
      ...
      $seleccionartitu = mysql_query("SELECT * FROM uaf_dispositivo_almacenamiento_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_DAAT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM uaf_dispositivo_almacenamiento_articulo_d WHERE '$sesiontitu[ID_DAAT]' = ID_TITULO_DAAT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_DAAD'];
...
$dompdf->render();
$dompdf->stream("Wiki_Dispositivos de almacenamiento.pdf");
