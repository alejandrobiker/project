...
<body>
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
      ...
      $seleccionartitu = mysql_query("SELECT * FROM uaf_electronica_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_EAT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM uaf_electronica_articulo_d WHERE '$sesiontitu[ID_EAT]' = ID_TITULO_EAT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_EAD'];
...
$dompdf->render();
$dompdf->stream("Wiki_Electronica.pdf");
