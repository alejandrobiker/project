...
AT
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
      ...
      $seleccionartitu = mysql_query("SELECT * FROM certificacion_electronica_criptografica_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_CECAT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM certificacion_electronica_criptografica_articulo_d WHERE '$sesiontitu[ID_CECAT]' = ID_TITULO_CECAT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_CECAD'];
...
$dompdf->render();
$dompdf->stream("Wiki_certificacion electronica.pdf");
