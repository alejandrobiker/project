...
AT
   ...
   <img style="margin-top:-20px;" width="100%" src="../../utilidad/img/articulo.png">
      ...
      $seleccionartitu = mysql_query("SELECT * FROM estandarizacion_fiscalizacion_articulo_t ",$con);

      while($sesiontitu = mysql_fetch_array($seleccionartitu)){
           
      $codigoHTML.='<h3>';  $codigoHTML.= $sesiontitu['TITULO_EFAT'];  $codigoHTML.='</h3>';

      

      $seleccionardesc = mysql_query("SELECT * FROM estandarizacion_fiscalizacion_articulo_d WHERE '$sesiontitu[ID_EFAT]' = ID_TITULO_EFAT",$con);

      while($sesiondesc = mysql_fetch_array($seleccionardesc)){
           
      $codigoHTML.= $sesiondesc['DESCRIPCION_EFAD'];
...
$dompdf->render();
$dompdf->stream("Wiki_estandarizacion y fiscalizacion.pdf");
